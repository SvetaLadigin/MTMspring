...
class RunningName:
    ...
    @staticmethod
    def _get_suffix_from_num(number):
        suffix = ''
        suffix += string.ascii_lowercase[number % len(string.ascii_lowercase)]
        number /= len(string.ascii_lowercase)
        while number > 0:
            suffix += string.ascii_lowercase[number % len(string.ascii_lowercase)]
            number /= len(string.ascii_lowercase)
    ...
    def __call__(self):
        name = '%s %s' % (self.prefix, RunningName._get_suffix_from_num(self.index))
        self.index += 1
        return name
...
state_name_generator = RunningName('state')
judge_name_generator = RunningName('judge')
song_name_generator = RunningName('song')

def add_state(f, states):
    state = new_id(states)
    name = state_name_generator()
    song = song_name_generator()
...
def rounded_score(score):
    return math.ceil(Decimal(str(score*1000000)))
...
def cmp_scores(item1, item2):
    ...
    # HW tests
    score_diff =  rounded_score(item1[1]) - rounded_score(item2[1]);
    if score_diff == 0:
...
def calc_contest(states, judges, aud_perc):
    ...
    for state in scores:
        #print('judge (%d) "%s" %f' % (state, states[state].name, judge_scores[state]))
        #print('state (%d) "%s" %f' % (state, states[state].name, state_scores[state]))
    ...
    for state in sorted(scores.items(), cmp=cmp_scores):
        #print('final (%d) "%s" %f' % (state[0], states[state[0]].name, state[1]))
...
def run_friendly(f, test_num, states):
    ...
    #print('calc friendly %d' % test_num)
